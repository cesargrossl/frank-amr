...
float medir_distancia() {
    ...
    // Espera início do pulso no ECHO (timeout de segurança)
    double inicio, fim;
    int timeout = 0;
    while (gpioRead(ECHO) == 0) {
        inicio = gpioTick();
        if (++timeout > 100000) return -1; // sem leitura
    }
    ...
    double duracao = fim - inicio; // microssegundos
